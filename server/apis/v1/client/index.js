...
function this_module(options) {
	...
	api.route('/')
		.get(function (req, res) {
			...
			if(name)
			{
				...
				params.$or = [{$where:'var s=this.name+" "+this.surname; return '+name+'.every(function(r){return r.test(s)})'}, {email: email}];
			}
	...
	api.route('/:id')
		...
		.put(function (req, res) {
			...
			if(json.password)
				json.password = crypter.hash(req.body.password);
			return clientService.update(req.params.id, json).then(function(client){
				res.json(client);
			}, function(err) {
				if(err && err.code==11000) {
					var value = err.err&&err.err.match(/: "([^"]+)"/);
